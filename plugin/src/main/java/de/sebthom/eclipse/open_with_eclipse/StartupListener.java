...
import de.sebthom.eclipse.open_with_eclipse.prefs.OpenWithEclipsePreference;
...
public class StartupListener implements IStartup {
   ...
   public void earlyStartup() {
      ...
      RegistryHelper.purgeInvalidOpenWithRegistryEntries();

      if (OpenWithEclipsePreference.isCreateOpenWithRegistration() //
         && !RegistryHelper.hasOpenWithRegistryEntry()) {

         UI.run(() -> {
            final var menuEntryLabel = new ObservableRef<>(OpenWithEclipsePreference.getOpenWithContextMenuEntryLabel());
            ...
            final var dlg = new MessageDialog( //
               ...
               OpenWithEclipsePlugin.get().getSharedImage(Constants.IMAGE_ICON), // window icon
               NLS.bind("No \"{0}\" entry in the Windows right-click/context menu yet.", OpenWithEclipsePreference
            ...
            if (dlg.open() == IDialogConstants.OK_ID) {
               OpenWithEclipsePreference.setCreateOpenWithRegistration(true);
               OpenWithEclipsePreference.setOpenWithContextMenuEntryLabel(menuEntryLabel.get());
               OpenWithEclipsePreference.save();
               RegistryHelper.createOpenWithRegistryEntry(Constants.LAUNCHER_EXE, Constants.LAUNCHER_NAME);
            } else {
               OpenWithEclipsePreference.setCreateOpenWithRegistration(false);
               OpenWithEclipsePreference.save();
            }
         });
      }
